...
const DebitCardView: React.FC<IProps> = ({
   ...
   * @returns Credit card number with space after evry 4 digits
   */
  function creditCardNumberWithSpace(input: string) {
    let output = '';
    if (input) {
      const regx = input.match(/.{1,4}/g);
      if (regx) output = regx.join('  ');
    }
    return output;
  }
  /**
   *
   * @param input Credit Card Number
   * @returns Credit Card Number Last 4 digits
   */
    ...
    <View>
      <View style={styles.cardDetailsVisibilityContainer}>
        <TouchableHighlight underlayColor="transparent" onPress={onEyePress}>
          <View style={styles.cardDetailsVisibilityContainerInner}>
            ...
            />
            <Text style={styles.redEyeText}>
              {isVisible ? CONSTANTS.HIDE_CARD_NUMBER : CONSTANTS.SHOW_CARD_NUMBER}
            </Text>
          </View>
        </TouchableHighlight>
      </View>
      <View style={styles.cardContainer}>
        ...
        <View style={styles.cardNumberDetailsContainer}>
          ...
          {isVisible ? (
            <Text style={styles.debitCardNumber}>
              {accountDetails?.debit_card?.card_number
                ? creditCardNumberWithSpace(accountDetails?.debit_card?.card_number)
                : ''}
            </Text>
          ) : (
            <Text style={styles.debitCardNumber}>
              {creditCardNumberWithSpace(
                `${CONSTANTS.HIDDEN_CARD_DOTS}${getLastourDigits(
                  accountDetails?.debit_card?.card_number,
                )}`,
              )}
            </Text>
          ...
          <View style={styles.cvvExpiryContainer}>
            <Text
              style={
                styles.expiry
              }>{`${CONSTANTS.EXP_DATE_PREFIX}${accountDetails?.debit_card?.card_expiry}`}</Text>
            <Text style={styles.cvv}>{`${CONSTANTS.CVV_PREFIX}${
              isVisible ? accountDetails?.debit_card?.card_cvv : '***'
            }`}</Text>
          </View>
        </View>
        <View style={styles.cardProviderContainer}>
          <Image
            resizeMode="contain"
            source={IMAGE_ASSETS.ICON_VISA_WHITE}
...
const styles = StyleSheet.create({
  ...
  cardBrandLogo: {
    height: _scaleText(21).fontSize,
    width: _scaleText(74).fontSize,
  },
  ...
  cardProviderContainer: {
    position: 'absolute',
    bottom: _scaleText(24).fontSize,
    right: _scaleText(22).fontSize,
  },
  cardProviderImage: {
